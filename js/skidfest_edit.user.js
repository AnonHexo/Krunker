...
class Utilities {
    ...
    createSettings() {
        this.settings = {
            ...
            hideStreams: {
                ...
                set: (value) => {
                    window.streamContainer.style.display = value ? "none" : "inherit"
                }
            },
            hideMerch: {
                name: "Hide Merch",
            ...
            weaponZoom: {
                ...
                set: (value) => {
                    if (this.renderer) this.renderer.adsFovMlt = value;
                }
            },
            weaponTrails: {
                name: "Weapon Trails",
                ...
                set: (value) => {
                    if (this.me) this.me.weapon.trail = value;
                }
            },
            autoBhop: {
            ...
            audioVolume: {
                ...
                set: (value) => {
                    if (this.settings.playStream.audio) this.settings.playStream.audio.volume = value;
                }
            },
        ...
        let waitForWindows = setInterval(_ => {
            if (window.windows) {
                ...
                menu.gen = _ => {
                    var tmpHTML = `<div style='text-align:center'> <a onclick='window.open("https://github.com/AnonHexo/Krunker")' class='menuLink'>KrunkerHex Settings</center></a> <hr> </div>`;
    ...
    objectEntries(object, callback) {
        ...
        original_Object.entries(descriptors).forEach(([key, {
            value,
            get,
            set,
            configurable,
    ...
    patchScript() {
        ...
        console.groupEnd();
        /*Lemons1337*/
        string = string.replace(/\[(0x[a-zA-Z0-9]+,?)+]\['map']\(\w+=>String\['fromCharCode']\(\w+\)\)\['join']\(''\)/g, a => "'" + w.eval(a) + "'");
    ...
    deObfuscate() {
        const obfu = {
            ...
            xDire: {
                regex: /this\['(\w+)']=Math\['lerpAngle']\(this\['xDir2']/,
                pos: 1
            },
            yDire: {
                regex: /this\['(\w+)']=Math\['lerpAngle']\(this\['yDir2']/,
                pos: 1
            },
    ...
    onRender() {
        ...
        if (espVal === "walls" || espVal === "twoD") this.nameTags = undefined;
        else this.nameTags = true;
        ...
        if (this.settings.autoActivateNuke.val && this.me && Object.keys(this.me.streaks).length) {
            /*chonker*/
            this.ws.__send("k", 0);
        }
